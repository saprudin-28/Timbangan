 ...
 DMD::writePixel(unsigned int bX, unsigned int bY, byte bGraphicsMode, byte bPixel)
{
	...
	/*
	if (bX>=0 && bX<8  && bY>=0  && bY<=8  ){bX=bX+8;bY=bY+8;}
	if (bX>=16 && bX<24  && bY>=0  && bY<=8  ){bX=bX+8;bY=bY+8;}
	if (bX>=8 && bX<16  && bY>8  && bY<16 ){bX=bX-8;bY=bY-8;}
	if (bX>=24 && bX<32  && bY>8  && bY<16 ){bX=bX-8;bY=bY-8;}
	*/
    ...
    byte lookup = bPixelLookupTable[bX & 0b00000111];
  ...
  return;
    ...
    int strWidth = 0;
     // this->drawLine(bX -1 , bY, bX -1 , bY + height, GRAPHICS_INVERSE);
             //this->drawLine(bX -1 , bY, bX -1 , bY + height, GRAPHICS_NORMAL);
     for (int i = 0; i < length; i++) {
 
        int charWide = this->drawChar(bX+strWidth, bY, bChars[i], bGraphicsMode);
      if (charWide > 0) {
          strWidth += charWide ;
          //this->drawLine(bX + strWidth , bY, bX + strWidth , bY + height, GRAPHICS_INVERSE);// ono garise
...
void DMD::clearScreen(byte bNormal)
{
    if (bNormal) // clear all pixels
        memset(bDMDScreenRAM,0b11111111,DMD_RAM_SIZE_BYTES*DisplaysTotal);
    else // set all pixels
        memset(bDMDScreenRAM,0b00000000,DMD_RAM_SIZE_BYTES*DisplaysTotal);
}
...
*/
		   ...
		   //Dancuk=bDMDScreenRAM;
			 ...
			 // SPI.transfer(0b00000000);
			 // SPI.transfer(0b00000000);
       					//	SPI.transfer(0b00000000);  SPI.transfer(0b00000000); 
		}
...
int DMD::drawChar(const int bX , const int bY , const unsigned char letter, byte bGraphicsMode)
{
    ...
    for (uint8_t j = 0; j < width; j++) { // Width
	  ...
	  for (uint8_t i = bytes - 1; i < 254; i--) { // Vertical Bytes
       ...
       //uint8_t data = 0b11111111111111111111111111111111;
		  ...
		  for (uint8_t k = 0; k < 8; k++) { // Vertical bits
			...
			if ((offset+k >= i*8) && (offset+k <= height)) {
				...
				if (data & (1 << k)) {
                  ...
                  writePixel(bX + j+0, bY + offset + k+0, bGraphicsMode, true);
				  
				  
                } else {
                  writePixel(bX + j+0, bY + offset + k+0, bGraphicsMode, false);
                }
            }
          }
      }
...
//48 32 49 33 16 0 17 1 50 34 51 35 18 2 19 3